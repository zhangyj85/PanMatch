...
class PatchEmbed(nn.Module):
    def __init__(self, in_chans=1, embed_dim=64, pe='linear'):
        super().__init__()
        self.dim = embed_dim
        self.pe = pe
        ...
        # )
        # 替换为单层卷积+norm测试效果
        # 注意, BN 在 batch size=1 时, running_mean & running_var 容易产生 NAN 导致测试出现错误
        # 因此, 受限于显存大小, 建议将 BN 改为 InstanceNorm | LayerNorm
        self.proj = nn.Sequential(
            nn.Conv2d(in_chans, embed_dim, kernel_size=self.patch_size, stride=self.patch_size, padding=0, bias=False),
...
class CostPerceiverEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.patch_embed = PatchEmbed(in_chans=self.cfg.cost_heads_num, embed_dim=cfg.cost_latent_input_dim, pe=cfg.pe)
...
class MemoryEncoder(nn.Module):
    ...
    def forward(self, fmap1, fmap2, data, context=None):
        cost_volume = self.corr(fmap1, fmap2)
