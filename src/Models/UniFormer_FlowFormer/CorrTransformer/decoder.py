...
class MemoryDecoder(nn.Module):
    ...
    def upsample_flow(self, flow, mask, scale=8):
        """ Upsample flow field [H/14, W/14, 2] -> [H/2, W/2, 2] using convex combination """
        N, _, H, W = flow.shape
    ...
    def forward(self, cost_memory, context, data={}, flow_init=None):
        ...
        if flow_init is not None:
            ...
            flow_init = F.interpolate(flow_init/8, scale_factor=1/8, mode="bilinear", align_corners=True)   # 注意这里的尺度关系, 输入的参考光流被缩放到原来的 1/8
        ...
        for idx in range(self.depth):
            ...
            # info = delta_flow[:, 2:]
            # flow_up, info_up = self.upsample_data(coords1 - coords0, info, up_mask)
            # flow_predictions.append(flow_up)
...
class MemoryDecoderwithProb(MemoryDecoder):
    def __init__(self, cfg):
        super().__init__(cfg)
        # 更新网络结构, 预测分布参数
        self.variable = {}
        self.update_block.flow_head.register_forward_hook(
            self.get_FlowHead_hook()
        )
        # self.info_conv = nn.Conv2d(256, 4, 3, padding=1)
    ...
    def get_FlowHead_hook(self):
        # ref: https://blog.csdn.net/Brikie/article/details/114255743
        # &: https://github.com/isl-org/DPT/blob/main/dpt/vit.py
        def hook(module, tuple_input, tuple_output):
            x = tuple_input[0]
            y = module.relu(module.conv1(x))
            self.variable['FlowHead_intermediate_variable'] = y
        return hook
    ...
    def forward(self, cost_memory, context, data={}, flow_init=None):
        ...
        for idx in range(self.depth):
            ...
            coords1 = coords1 + delta_flow
            info = self.info_conv(self.variable['FlowHead_intermediate_variable'].detach())
            flow_up, info_up = self.upsample_data(coords1 - coords0, info, up_mask)
            flow_predictions.append(flow_up)
            info_predictions.append(info_up)
